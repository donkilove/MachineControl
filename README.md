...
- **移动执行器 `MachineWorker`** —— 逐条指令发送 + ACK 等待（2s 窗口），成功后等待到位时间，整轮最多尝试 2 次（含 1s 重试间隔），协作式取消
...
- **固件依赖（未实机确认）**：序列中途失败后的整轮重发，会向可能正处于移动/已移动状态的机台再次发送 `AT+IO=00` 复位指令——**「运动中重复复位」的幂等性依赖机台固件行为，本库未做假设，使用方须向固件方确认**。
